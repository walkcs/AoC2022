...
realIn = '''LdHVLDLDdHdtLMhcqCqGWcWg
...
alpha_dict = {k: ord(k) for k in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'}

def totaPrio(sample):
    elfList = sample.split('\n')
    tp = 0
    for e in elfList:
        midPoint = int(len(e)/2)
        h1 = e[0:midPoint]
        h2 = e[midPoint:]
        h1s = set(h1)
        h2s = set(h2)
        commonEl = list(h1s & h2s)
        if alpha_dict[commonEl[0]]>96:
            p = alpha_dict[commonEl[0]]-96
        else:
            p = alpha_dict[commonEl[0]]-64+26
        tp += p
    return tp
print(totaPrio(sample))
print(totaPrio(realIn))

#part2
def totaPrio2(sample):
    elfList = sample.split('\n')
    tp = 0
    i = 0
    while i < len(elfList):
        ...
        i += 3
    return tp
print(totaPrio2(sample))
print(totaPrio2(realIn))